...
export const UpdateProfPic = async (jwt_token, picFile, setUserPic) => {
  try {
    ...
    const abc = data.profilePic.data.data

    let TYPED_ARRAY = new Uint8Array(abc);
    const STRING_CHAR = String.fromCharCode.apply(null, TYPED_ARRAY);
  
    const myurl = await STRING_CHAR.toString('')
    console.log(myurl)
    // let img = new Image()
    //     img.onload = () => {
    ...
    //   })
    // }
    // const imagePic = blobToImage(data.profilePic)
    // let imageData = data.profilePic.data.data;
    // let imageData = data.profilePic
    // let TYPED_ARRAY = new Uint8Array(imageData);
    // const STRING_CHAR = String.fromCharCode.apply(null, imageData);
    // let base64String = btoa(STRING_CHAR);
